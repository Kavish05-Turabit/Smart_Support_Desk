...
LOGGING_CONFIG["formatters"]["default"]["fmt"] = "%(levelprefix)-9s %(message)s"
LOGGING_CONFIG["formatters"]["access"]["fmt"] = '%(levelprefix)-9s %(client_addr)s - "%(request_line)s" %(status_code)s'
...
async def lifespan(app: FastAPI):
    ...
    try:
        await r.ping()
        sess.redisDb = r
        logger.info("Redis session successfully connected")
    except RedisConnectionError:
        sess.redisDb = sess.MockRedis()
        await r.close()
        logger.warning("Redis not available currently, using placeholder.")
