...
async def create_note(ticket_id: int, note_in: NoteCreate, db: DBdependency,
    ...
    try:
        ...
        logger.info(f"User {user.employee_id} create a Note {new_note.note_id} for Ticket {ticket_id}")
        await redis_client.delete(f"notes_for:{ticket_id}")
        return new_note

    except Exception as e:
        db.rollback()
        logger.info(f"Error creating Note for Ticket {ticket_id} by User {user.employee_id}")
        raise HTTPException(
...
async def edit_note(note_id: int, note_in: NoteCreate, db: DBdependency,
    ...
    try:
        ...
        logger.info(f"User {user.employee_id} updated Note {note_id} for Ticket {note.ticket_id}")
        return {"message": "Note Updated Successfully"}

    except Exception as e:
        db.rollback()
        logger.error(f"Note updation failed for Note {note_id} by user {user.employee_id}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
